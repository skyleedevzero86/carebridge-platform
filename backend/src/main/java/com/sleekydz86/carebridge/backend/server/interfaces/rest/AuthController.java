...
public class AuthController {
    ...
    public ResponseEntity<Map<String, String>> logout(Authentication authentication) {
        ...
        return ResponseEntity.ok(Map.of("message", "ë¡œê·¸?„ì›ƒ?˜ì—ˆ?µë‹ˆ??"));
    }

    private AuthenticatedUserPrincipal current(Authentication authentication) {
        if (authentication == null || !(authentication.getPrincipal() instanceof AuthenticatedUserPrincipal principal)) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "?¸ì¦ ?•ë³´ê°€ ?†ìŠµ?ˆë‹¤.");
        }
        return principal;
    }

    public record RegisterRequest(
            @NotBlank(message = "?„ì´?”ë? ?…ë ¥??ì£¼ì„¸??")
            @Size(min = 3, max = 30, message = "?„ì´?”ëŠ” 3???´ìƒ 30???´í•˜?¬ì•¼ ?©ë‹ˆ??") String username,
            @NotBlank(message = "?´ë¦„???…ë ¥??ì£¼ì„¸??")
            @Size(min = 2, max = 20, message = "?´ë¦„?€ 2???´ìƒ 20???´í•˜?¬ì•¼ ?©ë‹ˆ??") String displayName,
            @NotBlank(message = "ë¹„ë?ë²ˆí˜¸ë¥??…ë ¥??ì£¼ì„¸??")
            @Size(min = 8, max = 40, message = "ë¹„ë?ë²ˆí˜¸??8???´ìƒ 40???´í•˜?¬ì•¼ ?©ë‹ˆ??") String password
    ) {}
    public record LoginRequest(
            @NotBlank(message = "?„ì´?”ë? ?…ë ¥??ì£¼ì„¸??") String username,
            @NotBlank(message = "ë¹„ë?ë²ˆí˜¸ë¥??…ë ¥??ì£¼ì„¸??") String password
    ) {}
