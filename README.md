...
병원·검사실 환경에서는 EMR만 쓰는 것이 아니라, **장비 전용 프로토콜(TCP, 키-밸류, HL7 스타일 등)**으로 들어오는 데이터를 중간 계층에서 받아 정리해야 하는 경우가 많습니다. 이 프로젝트는 그 흐름을 단순화해 보여 주기 위해:

- **동일한 수신 경로**로 실제 TCP 클라이언트와 내장 시뮬레이터 트래픽을 처리하고
...
교육·포트폴리오용 샘플입니다. 실제 의료기기·HL7 연동·규제 요구사항을 대체하지 않습니다.
